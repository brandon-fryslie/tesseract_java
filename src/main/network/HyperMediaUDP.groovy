 ...
 * <p>
 * Originally written by: Cousot Stéphane - stef@ubaa.net
 * Updated by: Brandon Fryslie and released according to original license terms
 */
...
public class HyperMediaUDP implements Runnable {
  ...
  private boolean log = false;  // enable/disable output log
  ...
  private String timeoutHandler = "timeout";
  ...
  private HyperMediaUDP(Object owner, int port, String ip) {
    ...
    // TODO: make this not depend on Processing - register a callback or something
  ...
  private boolean isBroadcast() {
    ...
    try {
      result = (ucSocket != null) && ucSocket.getBroadcast();
    } catch (SocketException e) {
      error(e.getMessage());
    } finally {
      return result;
