...
public class ParticleClip extends AbstractClip {
  ...
  public void addParticle() {
    ...
    // TODO: figure out how this clip can work w/o needing direct access to the stage
    // Expose stage dimensions on TesseractMain?
    float locX = _myMain.getStage().getMinX() + (_myMain.getStage().getMaxW() / 2);
    float locY = _myMain.getStage().getMinY() + (_myMain.getStage().getMaxH() / 2);
    float locZ = _myMain.getStage().getMinZ() + (_myMain.getStage().getMaxD() / 2);
    ...
    int particleColor = _myMain.color(rRed, rBlue, rGreen);
    ...
    _particles.add(new Particle(theLoc, particleColor, _pSize, _pRamp, theSpeed, theAccel));
